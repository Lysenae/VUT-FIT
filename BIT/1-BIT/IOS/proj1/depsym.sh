...
function print_dependancies
{
  if [ $1 == false ]; then
    ...
    o1=$( echo $2 | sed 's/^.*\///' | sed 's/\./D/')
    o2=$( echo $3 | sed 's/^.*\///' | sed 's/\./D/')
    printf "%s -> %s [label=\"%s\"];\n" $o1 $o2 $4
  fi
}
...
do
  if ! [ -e $file ]; then
    print_err "Subor $file neexistuje"
    exit 1
  fi
done
...
if [[ -n $OBJ_ID ]]; then
  ...
  if [[ $obj_found == false ]]; then
    print_err "Objekt $OBJ_ID nebol najdeny"
    exit 1
  fi
fi
