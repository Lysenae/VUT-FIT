...
DROP TABLE PLANT_TABLE CASCADE CONSTRAINTS;

DROP SEQUENCE pres_seq;

DROP PROCEDURE zadost_o_presun;
DROP PROCEDURE nabor;

DROP MATERIALIZED VIEW zam_role;
...
-- auto_increment PK pro presuny
CREATE TRIGGER tr_pk
BEFORE INSERT ON Presun
  FOR EACH ROW
  BEGIN
    :new.presunID := CONCAT('T',TO_CHAR(pres_seq.NEXTVAL,'FM0999'));
  END;
/

-- PROCEDURE
-- pridani zadosti o presun s kontrolou zda je zadajici zodpovida za zarizeni a zda byl zadan nejaky duvod, vyuziva incrementacni trigger
CREATE PROCEDURE zadost_o_presun (claimant IN Presun.zadatel%TYPE, device IN Presun.zarizeni%TYPE, reason IN Presun.duvod%TYPE) 
IS
  foundZar Presun.zarizeni%TYPE;

BEGIN
  BEGIN
    SELECT zarID INTO foundZar
        FROM Zarizeni
        WHERE zarID = device and zodpovida = claimant;
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
      Raise_application_Error (-20004, 'Zadajici nezodpovida za dane zarizeni. Tedy nemuze zadat o jeho presun.');
  END;
  IF reason = '' or reason IS NULL THEN
    Raise_application_Error (-20005, 'Bezduvodna zadost o presun.');
  END IF;

  INSERT INTO Presun(presunID, zarizeni, zadatel, duvod, provedeno, cas_zadosti, odkud, kam)
    VALUES (NULL, device, claimant, reason, TO_DATE('','yyyy-mm--dd hh24:mi'), TO_DATE(SYSDATE, 'yyyy-mm--dd hh24:mi'), 'M-14-01', 'M-13-01');
END;
/

-- pro ktera oddeleni bychom meli hledat zamestnance?
SET serveroutput ON format WRAPPED;   -- zapnuti presmerovani DM
CREATE PROCEDURE nabor (wanted NUMBER)
IS
  pocet NUMBER(3);
  CURSOR zam 
  IS
    SELECT o.nazev, COUNT(z.login) AS pocet
      FROM Oddeleni o
        LEFT JOIN Mistnost m ON m.oddeleni = o.oddID 
          LEFT JOIN Zamestnanec z ON z.pracoviste = m.roomID
      GROUP BY o.nazev
    ;
  TYPE zam_tt IS TABLE OF zam%ROWTYPE;
  tmp_zam zam_tt;
  diff NUMBER;
BEGIN
  OPEN zam;
  FETCH zam BULK COLLECT INTO tmp_zam;
  CLOSE zam;
  
  FOR i IN 1..tmp_zam.COUNT LOOP
    IF tmp_zam(i).pocet < wanted THEN
      diff := wanted - tmp_zam(i).pocet;
      IF diff < 5 THEN
        DBMS_OUTPUT.put_line('Shanime '||TO_CHAR(diff)||' zamestnance pro oddeleni '||tmp_zam(i).nazev);
      ELSE
        DBMS_OUTPUT.put_line('Shanime '||TO_CHAR(diff)||' zamestnancu pro oddeleni '||tmp_zam(i).nazev);
      END IF;
    END IF;
  END LOOP;
...
INSERT INTO Porucha(poruchaID, k_oprave, technik, oznamujici, popis, cas_oznam, zprava_opravy)
  VALUES ('P0002', 'D0001', 'Z4', 'Z4', 'Nedetekuje pam. kartu', TO_DATE('2015-02-27', 'yyyy-mm--dd'), 'Vyměněn slot pro kartu');
INSERT INTO Porucha(poruchaID, k_oprave, technik, oznamujici, popis, cas_oznam, zprava_opravy)
  VALUES ('P0001', 'D0005', 'Z4', 'Z2', 'Vadne pixely', TO_DATE('2015-03-28', 'yyyy-mm--dd'), 'Provedena reklamace');

-- demonstrace auto_increment triggeru
INSERT INTO Presun(presunID, zarizeni, zadatel, duvod, provedeno, cas_zadosti, odkud, kam)
  VALUES ('NULL', 'D0001', 'Z3', 'Nepouzivam', TO_DATE('2015-04-03 9:20','yyyy-mm--dd hh24:mi'), TO_DATE('2015-03-28 18:10', 'yyyy-mm--dd hh24:mi'), 'M-14-01', 'M-13-01');
  
CALL zadost_o_presun('Z4','D0002','Zbytecne velka');
-- Toto se provede zavolanim zadost_o_presun()
--   INSERT INTO Presun(presunID, zarizeni, zadatel, duvod, provedeno, cas_zadosti, odkud, kam)
--    VALUES (NULL, 'D0002', 'Z4', 'Zbytecne velka', TO_DATE('','yyyy-mm--dd hh24:mi'), TO_DATE('2015-03-28 18:10', 'yyyy-mm--ddhh24:mi'), 'M-13-01', 'M-21-01');

-- vyvolani chyb spojenych s zadost_o_presun
CALL zadost_o_presun('Z5','D0002','Zbytecne velka');  -- err 20005
CALL zadost_o_presun('Z4','D0002','');                -- err 20006
CALL zadost_o_presun('Z4','D0002',NULL);              -- err 20006

-- cislo urcuje kolik chceme mit zamestnancu v kazdem oddeleni, 6 ukazuje rozdil ve vystupni zprave
CALL nabor(6);
COMMIT;
...
-- 1 spojeni 3 tabulek
-- Pod kterym oddelenim ktery zamestnanec pracuje a kde ho najdeme?
...
EXPLAIN PLAN SET STATEMENT_ID = 'withoutINDEX' FOR
SELECT z.jmeno, z.prijmeni, p.k_oprave, COUNT(*) as Pocet_poruch
  FROM Zamestnanec z, Porucha p
  WHERE z.zamID = p.technik
  GROUP BY p.k_oprave, z.jmeno, z.prijmeni
;

CREATE INDEX poruchy ON Porucha(k_oprave,technik);

EXPLAIN PLAN SET STATEMENT_ID = 'withINDEX' FOR
SELECT z.jmeno, z.prijmeni, p.k_oprave, COUNT(*) as Pocet_poruch
  FROM Zamestnanec z, Porucha p
  WHERE z.zamID = p.technik
  GROUP BY p.k_oprave, z.jmeno, z.prijmeni
...
-- GRANT prava druhymu clenovi tymu

GRANT ALL ON Mistnost TO xvecer18;
GRANT ALL ON Zarizeni TO xvecer18;
GRANT ALL ON Zamestnanec TO xvecer18;
GRANT ALL ON Oddeleni TO xvecer18;
GRANT ALL ON Presun TO xvecer18;
GRANT ALL ON Porucha TO xvecer18;
GRANT ALL ON Kancelar TO xvecer18;
GRANT ALL ON Ucebna TO xvecer18;

-- VIEW 
-- Jakou roli ma ktery zamestnanec?
CREATE MATERIALIZED VIEW zam_role BUILD IMMEDIATE REFRESH COMPLETE ON DEMAND AS
SELECT z.jmeno, z.prijmeni, p.popis AS role
  FROM Zamestnanec z, Prava p
  WHERE p.pravaID = z.prava
;
GRANT ALL ON zam_role TO xvecer18;
